...
;; ERROR
(DEFUN f_error (arg) (ERROR "JKIT.EMBED: f_error ~W" arg))
...
(DEFINE-COMPILER-MACRO wrap (val)
  `(IF ,val 'true 'false))
(DEFUN <unwrap-error> (x) (ERROR "JKIT.EMBED.CORE: unwrap: ~A is not a boolean" x))
(DEFUN unwrap (q_bool)
...
(DEFUN concat (&REST seqs)
  (WHEN (NULL seqs)
    (ERROR "JKIT.EMBED.CORE: concat: call with no arg"))
...
(DEFUN abort () (ERROR "JKIT.EMBED.CORE: aborted by (abort)"))
