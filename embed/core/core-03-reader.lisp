  ...
  (COND
   ...
   ((AND (CONSP V1) (EQ (CAR V1) '|@p|) (CONSP (CDR V1)) (CONSP (CDDR V1))
     ...
     (LET ((arity (LIST-LENGTH (CDR V1))))
       ...
       (proc_specialforms (CONS (SVREF JKIT.ALGEBRAIC.CORE::%TUPLE-CONVERSION-VECTOR% arity) (CDR V1)))))
   ...
   ((AND (CONSP V1) (SYMBOLP (CAR V1))
         (GET (CAR V1) 'JKIT.ALGEBRAIC.XDATA::|%data_tag%|)
     ...
     ;;(PRINT V1)
     ;;(UNLESS (GET (CAR V1) 'JKIT.ALGEBRAIC.XDATA::|%data_tag%|) (WARN "xi: (~A ...) is unknown type constructor" (CAR V1)))

     (LET* ((ctor (CAR V1))
            (data-tag (GET ctor 'JKIT.ALGEBRAIC.XDATA::|%data_tag%|))
            (data-arity (GET ctor 'JKIT.ALGEBRAIC.XDATA::|%data_arity%|))
            (args (CDR V1))
            (n (LENGTH args)))
       (proc_specialforms
