...
(let* ((*readtable* JKIT.EMBED.CORE:*qi-readtable*))
  (set-dispatch-macro-character 
   #\# #\{ 
   #'jkit.base:|#{-READER|))

;; (DEFUN JKIT.EMBED.CORE:QILOAD (filename)
;;   (let ((*package* (find-package :Q))
;;         (*readtable* JKIT.EMBED.CORE:*qi-readtable*))
;;     (JKIT.EMBED.CORE:|load| filename)))
...
(defun qi-reader (stream &key transformer pre-execute)
  (do (c 
      ...
      (end-flag
       (let* ((raw-chars (nreverse tmp))
              ...
              (raw-parsed (let ((*<caller-package>* *package*)
                                (*package* (find-package :JKIT.MSPACE))
                                (*readtable* JKIT.EMBED.CORE:*qi-readtable*)
                                (errstr "JKIT.EMBED.CORE: parse: failure: ~%~%~{~C~} ..."))
                            (JKIT.EMBED.CORE::|compile| 'JKIT.EMBED.CORE::|<st_input>| chars errstr)))
                                        ;`(progn ,@(mapcar (lambda (x) `(XI.CORE::|eval| ',x))
                                        ;                  parsed))))
              (parsed (if (JKIT.EMBED.CORE::%need-trans-p% raw-parsed)
                        (JKIT.EMBED.CORE::%trans% raw-parsed)
                        raw-parsed))
              (form (if pre-execute
                      (let ((|*<definition-only-p>*| t))
                        (JKIT.EMBED.CORE::%load% parsed)
                        |*<definition-only-p>*|)
                      `(JKIT.EMBED.CORE::%load% ',parsed))))
         ;(WARN "~D" form)
         form
    ...
    (unless (characterp (setq c (read-char stream)))
      (error "JKIT.EMBED.CORE: unexpected end of stream"))
...
;; OBSOLETE
'(jkit.base:define-lpar-backslash-reader "XI" (stream)
  (FORMAT T "\\XI is OBSOLETE~%")
  (qi-reader stream))

;; OBSOLETE
'(jkit.base:define-lpar-backslash-reader "XDEF" (stream)
  (FORMAT T "\\XDEF is OBSOLETE~%")
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")"))))

;; OBSOLETE
'(jkit.base:define-lpar-backslash-reader "XLAMBDA" (stream)
  (FORMAT T "\\XLAMBDA is OBSOLETE~%")
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def xi_lambda " chars ")"))))

;; OBSOLETE
'(jkit.base:define-lpar-backslash-reader "XDEFUN" (stream)
  (FORMAT T "\\XDEFUN is OBSOLETE~%")
  (let ((*<LOCALLY-P>* T))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))
;; OBSOLETE
'(jkit.base:define-lpar-backslash-reader "XDEFMACRO" (stream)
...
;; OBSOLETE
'(jkit.base:define-lpar-backslash-reader "X" (stream)
...
(jkit.base:define-lpar-backslash-reader "XENV" (stream)
  (qi-reader stream))

(jkit.base:define-lpar-backslash-reader "DEF" (stream)
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")"))))

(jkit.base:define-lpar-backslash-reader "LAMBDA" (stream)
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def xi_lambda " chars ")"))))

(jkit.base:define-lpar-backslash-reader "DEFUN" (stream)
  (let ((*<LOCALLY-LET-OP>* 'LABELS))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))

(jkit.base:define-lpar-backslash-reader "DEFMACRO" (stream)
  (let ((*<LOCALLY-LET-OP>* 'LABELS)
        (*<DEF-OP>* 'DEFMACRO))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))

(jkit.base:define-lpar-backslash-reader "LET" (stream)
  (let ((*<LOCALLY-LET-OP>* 'FLET)
        (*<DEF-OP>* NIL))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))

(jkit.base:define-lpar-backslash-reader "LETREC" (stream)
  (let ((*<LOCALLY-LET-OP>* 'LABELS)
        (*<DEF-OP>* NIL))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))
...
(jkit.base:define-lpar-backslash-reader "\\" (stream)
  (<deconstruct-lambda>
   (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def xi_lambda -> (" chars "))")))))
...
;;OBSOLETE
;;(jkit.base:define-lpar-backslash-reader "XDECL" (stream)
;;  (qi-reader stream :transformer (lambda (chars) (<trans> "(decl " chars ")"))))

;;OBSOLETE
;;(jkit.base:define-lpar-backslash-reader "X-YACC" (stream)
;;  (qi-reader stream))


;; (jkit.base:define-lpar-backslash-reader "X-PROLOG" (stream)
;;   (do (c 
;;        tmp
;;        escape-flag
;;        comma-flag
...
;;                         (JKIT.EMBED.CORE::|compile| 'JKIT.EMBED.CORE::|<horn_clauses>| chars errstr))))
;;          (if (JKIT.EMBED.CORE::%need-trans-p% parsed)
;;            `(JKIT.EMBED.CORE:|s-prolog| ',(JKIT.EMBED.CORE::%trans% parsed))
;;            `(JKIT.EMBED.CORE:|s-prolog| ',parsed))))    
...
(jkit.base:register-reader-registerer :embed '<register-readers>)
